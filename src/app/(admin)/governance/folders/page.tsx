...
const parseContractError = (err: any, fallback: string) => {
  ...
  const raw = messageCandidates.find((msg) => typeof msg === 'string' && msg.trim().length > 0)
  if (raw) {
    if (raw.includes('missing revert data')) {
      return 'Transaction reverted before estimation. Please ensure the contract state allows this action and try again.'
    }
    if (raw.includes('CALL_EXCEPTION')) {
      return 'Contract call failed. Please confirm the folder is approved and you have sufficient permissions.'
    }
    return raw
  }
...
const FolderRegistryPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const message = parseContractError(err, 'Failed to load folders')
  ...
  const refreshFolders = async () => {
    ...
    try {
      ...
      const message = parseContractError(err, 'Failed to refresh folders')
      showNotification({ message, variant: 'danger' })
      setLastError(message)
    } finally {
      setFetching(false)
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      const message = parseContractError(err, 'Unable to create folder (connect wallet?)')
      showNotification({ message, variant: 'danger' })
      setLastError(message)
    }
  }
  ...
  const handleAllocation = async () => {
    ...
    try {
      ...
      const message = parseContractError(err, 'Unable to set allocation')
      showNotification({ message, variant: 'danger' })
      setLastError(message)
    }
  }
  ...
  const openFolderDetail = async (folder: FolderInfo) => {
    ...
    try {
      ...
      const message = parseContractError(err, 'Failed to fetch members')
      showNotification({ message, variant: 'danger' })
      setLastError(message)
      setMembers([])
    } finally {
